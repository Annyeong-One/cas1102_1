 ...
 */
template <Numeric T>
T get_max_numeric(const T& a, const T& b) {
    return (a > b) ? a : b;
}
 ...
 */
bool all_equal() {
    return true; // With zero arguments, we consider them all equal by definition.
}
 ...
 */
bool all_equal(const auto& value) {
    return true;
}
...
int main() {
    ...
    /*
    get_max_numeric requires arguments satisfying the concept Numeric
    which restricts T to be an arithmetic type,
    which std::string is not.
    */
    std::cout << std::endl;
...
//    std::cout << "Accumulated floats: " << integer_acc.get_total() << std::endl;
    /*
    Accumulator template requires T to be an integral type (by std::integral concept) which float is not.
    */
