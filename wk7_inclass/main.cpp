...
constexpr LightColor next(LightColor c) {
    ...
    // but is the description intended? shouldnt it be something like
    // switch(c){
    // case LightColor::Red:
    //     return LightColor::Green;
    // case LightColor::Green:
...
std::string describe(CLightRef tl) {
    ...
    // A. 'advance' is used to modify the value tl has, while
    //    'describe' only needs to read the value of tl without modifying
    std::string s;
